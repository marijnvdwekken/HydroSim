...
def get_controls(clients: dict[str, ModbusTcpClient], en: epanet) -> dict:
    try:
        ...
        for zone, client in clients.items():
            ...
            if pump_count > 0: 
                pump_registers = client.read_holding_registers(address=1000, count=pump_count * 2).registers  # has to be address=... weird as fuck...
            ...
            if valve_count > 0: 
                valve_registers = client.read_holding_registers(address=2000, count=valve_count * 2).registers  # has to be address=... weird as fuck...
